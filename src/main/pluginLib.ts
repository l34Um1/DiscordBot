...
export default class PluginLibrary {
  ...
  public chat: DiscordClient['chat']
  /** Whisper `msg` to `userId` */
  public whisper: DiscordClient['whisper']
  /** Join `channelIds` */
  public join: DiscordClient['join']
  /** Leave `channelIds` */
  public part: DiscordClient['part']
  ...
  private evHandler: EverythingHandler

  constructor(client: DiscordClient, data: Data, commander: Commander, evHandler: EverythingHandler) {
    ...
    this.chat = this.client.bind(this.client)
    this.whisper = this.evHandler.whisper.bind(this.client)
