...
import { Client as DiscordClient, GuildMember, User, Guild, Message } from 'discord.js'
...
export interface CommandAlias {
  ...
  whitelist?: Array<GuildMember['id']>
  /** List of user ids that may not use this alias at all */
  blacklist?: Array<GuildMember['id']>
}
...
type CommandAliasLike = {
  ...
  whitelist?: Array<GuildMember['id']>
  blacklist?: Array<GuildMember['id']>
} | DeepReadonly<{ userlvl?: Userlvl, whitelist?: Array<GuildMember['id']>, blacklist?: Array<GuildMember['id']> }>
...
export interface Handlers {
  call: {
    default: Array<{
      params: string
      handler: (guild: Guild, member: GuildMember, params: any, extra: Extra) => Promise<AdvancedMessage | string | void>
    }>
    [group: string]: Array<{
      params: string
      handler: (guild: Guild, member: GuildMember, params: any, extra: Extra) => Promise<AdvancedMessage | string | void>
...
export default class Commander {
  ...
  public masters: readonly string[]
  ...
  constructor(client: DiscordClient, data: Data, masters: readonly string[]) {
    this.defaultAliases = {}
    this.paths = {}
    this.plugins = {}
    this.instances = {}
    ...
    this.client.on('message', this.onChat.bind(this))
  }
  ...
  public delAlias(guildId: string, alias: string): boolean {
    ...
    this.data.data[guildId].aliases.deletes[alias] = true
    return true
  }
  ...
  public modAlias(guildId: string, alias: string, options: Partial<CommandAliasSource>): ReadonlyCommandAlias | void {
    ...
    if (!this.data.data[guildId].aliases.aliases[alias]) {
      // Copy global alias
      if (this.data.data[guildId].aliases.deletes[alias]) return
      if (!this.defaultAliases[alias]) return
      this.data.data[guildId].aliases.aliases[alias] = deepClone(this.defaultAliases[alias])
    }
    this.data.data[guildId].aliases.aliases[alias] = deepClone({ ...this.data.data[guildId].aliases.aliases[alias], ...options })
    this.data.data[guildId].aliases.deletes[alias] = true
  ...
  public getAlias(guildId: string, alias: string): ReadonlyCommandAlias | void {
    ...
    if (!this.data.data[guildId].aliases.deletes[alias]) {
      return this.defaultAliases[alias] || this.data.data[guildId].aliases.aliases[alias]
    }
    return this.data.data[guildId].aliases.aliases[alias]
  }
  ...
  public getAliases(guildId: string): { [x: string]: ReadonlyCommandAlias } | void {
    ...
    for (const key in this.defaultAliases) {
      if (!this.data.data[guildId].aliases.deletes[key]) defaults[key] = this.defaultAliases[key]
    }
  ...
  public getAliasesById(guildId: string, pluginId: string): { [x: string]: ReadonlyCommandAlias } | void {
    ...
    for (const key in this.defaultAliases) {
      if (!this.data.data[guildId].aliases.deletes[key] && this.defaultAliases[key].target === pluginId) {
        res[key] = this.defaultAliases[key]
      }
    }

    const locals = this.data.data[guildId].aliases.aliases
  ...
  public isPermitted(alias: CommandAliasLike, member: GuildMember, options: IsPermittedOptions = {}) {
    ...
    if (userlvl < Userlvl.admin) {
      if (!options.ignoreBlackList && alias.blacklist && alias.blacklist.includes(member.id)) return false
      if (!options.ignoreWhiteList && alias.whitelist && alias.whitelist.includes(member.id)) return true
    }
  ...
  private async onChat(message: Message) {
    ...
    const alias = this.getAlias(message.guild.id, words[0])
    ...
    // Remove @user... from command calls if required
    if (!plugin.allowMentions) content = content.replace(/ ?<@!?\d+>/, '')
    words = content.split(' ')
    ...
    if (this.isPermitted(alias, member)) {
      const userlvl = this.getUserlvl(member)
      if (userlvl >= Userlvl.admin) {
        ...
        if (cooldown <= 0 || whisperCall) { // Passing
          const cooldowns = this.data.getData(guild.id, 'cooldowns') as CooldownData
          if (cooldowns && !whisperCall) {
            // Add entries to cooldowns
            if (alias.cooldown) cooldowns.shared[alias.target].push(Date.now())
            if (alias.userCooldown && alias.userCooldown > (alias.cooldown || 0)) cooldowns.user[alias.target][member.id].push(Date.now())
          }
