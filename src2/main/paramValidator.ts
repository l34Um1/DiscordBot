...
export default class ParamValidator {
  ...
  constructor(commander: Commander) {
    ...
    this.checkables = ['USER', 'COMMAND', '!COMMAND', 'PLUGIN', '!PLUGIN']
  }

  /** Generates user readable output when usage is not valid */
  public async validate(guildId: string, pluginId: string, words: string[], group = 'default'): Promise<ValidateResult> {
        ...
        for (let i = 0; i < params.length; i++) {
          ...
          if (typeof pure === 'string' && params[i].var && this.checkables.includes(pure)) {
            ...
            for (let ii = 0; ii < checks; ii++) {
              ...
              switch (pure) {
                case 'USER': {
                  const userId = word.match('(?<=^<@|^<@!)\d+(?=>$)')
                  if (!userId) return { pass: false, message: `@user format is required for user parameters (param ${i + ii + 1})` }
                  break
                }
                case 'COMMAND':
                  if (!this.commander.getAlias(guildId, word)) {
  ...
  public consoleInteract() {
    ...
    async function listener(this: ParamValidator, data: any) {
      ...
      if (str.startsWith('>')) {
        ...
        const res = await this.validate('VALIDATOR_TEST', 'VALIDATOR_TEST', str.slice(1).split(' '))
