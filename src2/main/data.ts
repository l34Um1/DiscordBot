...
export default class Data extends EventEmitter {
  ...
  constructor(client: DiscordClient, dataRoot: string, reserved = ['']) {
    super()
    this.data = {}
    ...
    this.client.on('guildCreate', this.onJoin.bind(this))

    // removed from a server
    this.client.on('guildDelete', this.onPart.bind(this))
  ...
  public autoLoad(name: string, defaultData: object, setDefaults = false) {
    ...
    for (const channel in this.client.guilds) { // Load for present channels
  ...
  public async unautoLoad(name: string, noUnload = false): Promise<Data['autoLoads']> {
    for (let i = 0; i < this.autoLoads.length; i++) {
      if (this.autoLoads[i].name === name) {
        ...
        const savePromises = []
        for (const channelId in this.client.guilds) { // Load for present channels
  ...
  private onJoin(guild: Guild) {
    for (const autoLoad of this.autoLoads) {
      if (!this.data[guild.id] || typeof this.data[guild.id][autoLoad.name] !== 'object') {
        this.load(guild.id, autoLoad.name, autoLoad.defaultData, autoLoad.setDefaults)
      }
    }
  }

  private onPart(guild: Guild) {
    for (const autoLoad of this.autoLoads) {
      if (this.data[guild.id] && typeof this.data[guild.id][autoLoad.name] === 'object') {
        this.save(guild.id, autoLoad.name, true).catch((err) => {
          logger.error(`[Data.autoLoad] Error unloading ${guild.id}`, err)
        })
      } else {
        logger.warn(`Data already unloaded: ${guild.id}/${autoLoad.name}`)
      }
    }
  }
}
